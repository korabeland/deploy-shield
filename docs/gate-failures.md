...
- **`No tests were found` / `ConfigError` (not a score failure):** Stryker by default copies the package into a relocated sandbox before running the tests. A service whose tests import a sibling workspace package (e.g. `services/example-service` importing `@deploy-shield/contracts`) can't resolve that sibling from the sandbox — the copy sits deeper in the tree, so the relative source alias misses and the workspace symlink doesn't reproduce — so every test file throws at import, Stryker sees zero tests, and the job dies. Fix: set `"inPlace": true` in that package's `stryker.config.json` so mutation runs in the real package directory, where workspace resolution matches `pnpm test`. This is why `services/example-service` sets it and `packages/contracts` (no sibling dependency) does not. In-place mode leaves `stryker-setup-*.js` and `.stryker-tmp/` behind — both are git-ignored.
