...
- **The project's Root Directory must be set** to the service path (`services/example-service`). `scripts/setup.sh` does this; there's no CLI flag, so it goes through the REST API. This is load-bearing for pnpm monorepos: the deploy workflow runs the Vercel CLI from the **repo root** rather than passing `--cwd <service>`, because with `--cwd` Vercel resolves workspace symlinks relative to the git root but joins them onto the cwd, yielding a doubled path (`services/example-service/services/example-service/node_modules/…`) and failing every deploy.
- **Never put test files under `api/`.** Vercel compiles every file in that directory into its own serverless function, so a colocated `handlers.test.ts` deploys as a live public endpoint. This repo keeps service tests in `src/` and ships a `.vercelignore` excluding `*.test.ts` as a second guard.
- **Entrypoints must use named HTTP-method exports** (`export function GET(request: Request)`) or `export default { fetch }`. A bare `export default function handler(request)` is read as the legacy `(req, res)` signature: the returned `Response` is discarded, the response is never ended, and every request hangs to a 504. It typechecks and passes unit tests either way — only the smoke test catches it.
- **Previews stay private; CI authenticates.** `scripts/smoke-test.sh` reaches protected deployments with a [Protection Bypass for Automation](https://vercel.com/docs/deployment-protection/methods-to-bypass-deployment-protection/protection-bypass-automation) secret stored as `VERCEL_AUTOMATION_BYPASS_SECRET`. Generate one with `PATCH /v1/projects/{id}/protection-bypass`. Without the secret a protected deployment answers 401/302 and the smoke test fails loudly rather than silently passing.
- **Production is public on Vercel's free plan, and you cannot turn that off.** Setting `deploymentType: "all"` is rejected with `428 invalid_sso_protection`; `all_except_custom_domains` is the ceiling and protects previews only. Assume anything this template deploys to production is world-readable — keep the deployed surface to endpoints you are content to expose, or run the project on a plan that supports production authentication.
